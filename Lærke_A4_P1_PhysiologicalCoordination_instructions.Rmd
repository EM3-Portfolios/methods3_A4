...
HR_m2_shuff <- lmerTest::lmer(HR_self_change ~ ((HR_self + diff) : Task) : Control + (1 + Task | ParticipantID), subset(dlonger, Study == "3"), REML = F, control = lmerControl(optimizer = "nloptwrap", calc.derivs = F))
...
# Starting from the wide format, create "surrogate" data set with the data from surrogate pairs
surrogate <- d1 %>% 
  group_by(Study, Task) %>%
  mutate(HR2 = HR2[sample(row_number())],
         HR_other = HR_other[sample(row_number())],
         Resp_self = Resp_self[sample(row_number())],
         Resp_other = Resp_other[sample(row_number())]
         )
...
```{r}
df1 <- select(d1, time, HR1, Resp1, Study, Task, ID)
df2 <- select(d1, time, HR2, Resp2, Study, Task, ID)
