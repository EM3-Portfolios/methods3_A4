...
data_preprocess <- function(filename, threshold = 3){
  ...
  d <- d %>% group(n = 400, method = "greedy") %>% 
    dplyr::summarise(time = mean(time, na.rm = T), #oh wait that's why we had to have ds after creating new columns etc :|
...
```{r plotting, inspecting and saving data}
...
plot_taskpergroup <- function(id, d1){
  p <- d1 %>% 
    ...
    subset(ID == id) %>%
    ggplot() + 
    geom_line(aes(time, HR1S, color ="HR1")) + 
    geom_line(aes(time, HR2S, color = "HR2")) + #plotting 2 lines
    labs(y = "HR") +
...
#unique IDs: "11" "12" "13" "14" "21" "25" "26" "27" "28" "29" "31" "32" "34" "35" "36" "37" "38" "39" "41" "42" "43" "44" "45" "46" "47" "48"
