...
- conservatively assess the presence of coordination between two signals in a controlled context
...
-Time is in minutes
-Subjects are pairs (Resp1 is the respiration data of the first person, Resp2 is their pair)
-Study1 last year, Study2 two years ago
-We have to make unique IDs
-There's 3 tasks, counterbalanced, for G1 isnot necessarily the same as for G2 (They don't follow the same order) Their hr and resp might be coordinated at the end of the tasks regardless of what the tasks were about.
-Remove outliers
-Do 1 file (not 1 pair, just 1 file), then run function on the whole thing
...
```{r}
# Load the libraries
library(tidyverse, CRAN, dplyr)
install.packages(groupdata2)
library(groupdata2)

# Load the file
singlefile <- readr::read_csv("/Users/al/RStudio/Semester 3/Experimental-Methods-3/Assignment 4/methods3_A4/data/Study1_G1_T1_Synchronous.csv",
...
# Plot
plot1 <- ggplot(data = singlefile) + #M: or d1 if you want to see it after downsampling
...
singlefile <- singlefile %>% #M: applying the function to the data (all the variables)
...
plot2 <- ggplot(data = singlefile) + #M: or d1 if you want to see it after downsampling
...
plot3 <- ggplot(data = singlefile) + #M: or d1 if you want to see it after downsampling
  geom_path(aes(time, HR1_c, color = "P1")) + 
  geom_path(aes(time, HR2_c, color = "P2")) + 
  labs(x = "time", y = "HR") + 
  theme_classic()+
  ggtitle("Scaled data without outliers")
...
plot4 <- ggplot(data = singlefile) + #M: or d1 if you want to see it after downsampling
  geom_path(aes(time, HR1, color = "P1")) + 
  geom_path(aes(time, HR2, color = "P2")) + 
  labs(x = "time", y = "HR") + 
  theme_classic()+
